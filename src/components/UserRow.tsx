        ...
        {user.role === "ADMIN" ? (
          ...
          <form action={nameAction} className="flex items-center gap-1">
            ...
            <button
              ...
              className="rounded-md border border-slate-300 px-2 py-1 text-xs text-slate-700 transition hover:bg-slate-100 disabled:opacity-50"
            >
              {t.save}
            </button>
            {nameState.error && (
              <span className="text-xs text-red-600">{nameState.error}</span>
            )}
          </form>
        )}
      </td>
